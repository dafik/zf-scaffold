...
class ZFscaffold_ZfTool_Renderer_View_Crud_Index extends ZFscaffold_ZfTool_Renderer_Abstract
{
    ...
    public function render()
    {
        ...
        $rowFields = array('<td align="center"><input class="checkbox" type="checkbox" name="del_id[]" value="<?= $model->get' . $tableDefinition->getPrimaryKeyColumns()[0]->getPhpName() . '(); ?>" /></td>');
        ...
        foreach ($tableDefinition->getColumns() as $field) {
            ...
            if (('CHAR' === substr($field->getType(), -4) || 'TEXT' === substr($field->getType(), -4)) && 'PASSWORD' !== $field->getColumnName()) {
                $searchableFields[$field->getColumnName()] = $field->getPhpName();
            }
            if ('ENUM' == $field->getType() || 'SET' == $field->getType()) {
                ...
                $columnFilters = '<?= $this->formSelect(\'' . $field->getColumnName() . '\', $this->param' . $field->getColumnName() . ', array(\'onchange\' => \'updateFilters(\'' . $field->getColumnName() . '\', this.options[this.selectedIndex].value)\'), ' . $optionsCode . '); ?>';
            } elseif (isset($tableDefinition->getForeignKeys()[$field->getColumnName()])) {

                /** @var $referenceData RelationMap */
                $referenceData = $tableDefinition->relationsFK[$field->getColumnName()];
                ...
                $columnFilters = '<?= $this->formSelect(\'' . $field->getColumnName() . '\', $this->param' . $field->getColumnName() . ', array(\'class\'=>\'form-control\',\'onchange\' => \'updateFilters(\\\'' . $field->getColumnName() . '\\\', this.options[this.selectedIndex].value)\'), array(\'\' => \'- - Change - -\') + Dfi_Propel_Adapter_Options::get(\'' . $refTableDefinition->getPhpName() . '\'));?>';
            ...
            $tmp = array('<th<?= (\'' . $field->getColumnName() . '\' == $this->sortField ? \' class="sort-field sort-\'.htmlspecialchars($this->param_so). \'"\':\'\'); ?>>');
            $tmp [] = '<a href="<?= $this->url($_GET + array(\'_sf\' => \'' . $field->getColumnName() . '\', \'_so\' => \'asc\')); ?>"><i class=\'glyphicon glyphicon-arrow-up\'></i></a>';
            $tmp [] = $field->getPhpName();
            $tmp [] = '<a href="<?= $this->url($_GET + array(\'_sf\' => \'' . $field->getColumnName() . '\', \'_so\' => \'desc\')); ?>"><i class=\'glyphicon glyphicon-arrow-down\'></i></a>';
            $tmp [] = $columnFilters;
            $tmp [] = '</th>';
            $headers[] = $this->formatLineArray($tmp);

            $align = ('INTEGER' == $field->getType() || 'FLOAT' == $field->getType()) && !isset($tableDefinition->getForeignKeys()[$field->getColumnName()]) ? ' align="right"' : '';
        ...
        $tmp = array('<td align="center"><a class="btn btn-info btn-xs" href="<?= $this->url(array_merge(' . $this->getVariable('routeParams') . ', array(\'action\' => \'update\', \'id\' => $model->get' . $tableDefinition->getPrimaryKeyColumns()[0]->getPhpName() . '())), null, true); ?>"><i class="glyphicon glyphicon-pencil"></i> Edit</a>');
        $tmp[] = '<a class="btn btn-danger btn-xs" onclick="return confirm(\'Confirm deletion!\');" href="<?= $this->url(array_merge(' . $this->getVariable('routeParams') . ', array(\'action\' => \'delete\', \'del_id\' =>$model->get' . $tableDefinition->getPrimaryKeyColumns()[0]->getPhpName() . '())), null, true); ?>"><i class="glyphicon glyphicon-trash"></i> Delete</a></td>';
