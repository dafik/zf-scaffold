...
class DfiPgsqlSchemaParser extends BaseSchemaParser
{
    ...
    protected function addColumns(Table $table, $oid)
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $column->getDomain()->setDescription($row['description']);
    ...
    protected function addIndexes(Table $table, $oid)
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            foreach ($arrColumns as $key => $intColNum) {
                ...
                $names['attname' . $key] = $row2['attname'];

            } // foreach ($arrColumns as $intColNum)

            if ($this->addVendorInfo) {
                $vi = $this->getNewVendorInfoObject(array_merge($row, $names));
